...
export function PaymentPage({ order }: PaymentPageProps) {
  ...
  >("pending");
  const [, setSnapToken] = useState<string | null>(null);
  ...
  const initiatePayment = async () => {
    ...
    try {
      ...
      // 1. Buat tipe untuk response dari Midtrans Snap
      type MidtransResult = {
        ...
        payment_type: string;
        fraud_status: string;
        [key: string]: unknown; // supaya tidak error kalau ada field tambahan
      };

      if (data.token) {
        setSnapToken(data.token);
        // @ts-expect-error karena window.snap tidak dikenal oleh TS
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3">
                ...
                <p className="text-sm text-muted-foreground text-center">
                  Ini adalah mode demo. Pembayaran tidak akan diproses secara
                  nyata.
                </p>
              </div>
